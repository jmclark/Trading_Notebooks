...
def direction_evaluation(y_pred, y_test, y_train):
	y_pred = list(y_pred)
	y_test = list(y_test)
	y_train = list(y_train)

	correct_direction_percent = 0
	...
	for i, pred in enumerate(y_pred):
		...
		if np.sign(single_day_change) == np.sign(predicted_change):
			correct_direction_percent += 1
		
		correct_direction_percent /= len(y_pred)
			
	return correct_direction_percent
...
def evaluate_model_AA001_day_ahead(y_pred, y_test, y_train, trade_percentage):
	...
	return single_day_percentages, single_day_pred_changes, cumulative_percentages, sum_percent_trading_error, correct_direction_percent